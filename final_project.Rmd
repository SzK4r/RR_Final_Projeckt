...
In the referenced study, different levels of AI awareness among students were reported:

84.6% reported understanding what artificial intelligence means.
12% claimed they do not know what AI is.
3.4% chose not to answer or expressed uncertainty.
Further breakdown showed:

36.1% have an average awareness of AI.
19.2% combined are very well informed (2.4%) and well informed (16.8%).
44.7% combined are less informed, including poorly informed (35%) and very poorly informed (9.7%).
...
# 9. Relation of Student Knowledge about AI to Their Views on Various AI-related Statements
...
## 9.1 Analysis of Mean Knowledge Scores by Information Source Usage
...
## 9.3 Visualization of Agreement Levels on AI Statements
...
# 10. Analysis of Students' Feelings About the Development of AI
...
![Students' Feelings about the Development of AI](path/to/your/pie_chart.png)
...
# Enhance the layout
fig <- fig %>% layout(title = 'Students perception of AI',
                      showlegend = TRUE,
...
# 11. Comparison of Students' Perception of AI
...
# 12. Comparison of AI Knowledge Levels Over Time
...
- There has been a notable increase in the percentage of "very well informed" individuals across both genders in recent years.
- The percentage of individuals who are "very poorly informed" has decreased, indicating improved awareness and understanding of AI.
- Gender disparities in AI knowledge that were evident in earlier data appear to be narrowing, with more equitable knowledge distribution in the latest data.

This structured approach allows readers to delve into the details of the analysis and easily understand the comparative results, maintaining a clear narrative while providing access to the underlying data and methodologies.

<details>
<summary>Click here to view the generated table</summary>
```{r, echo=TRUE,warning=FALSE,message=FALSE,error=FALSE}
# Summarize the data
table_data <- data3 %>%
  group_by(Q12.Gender, Q1.AI_knowledge) %>%
  summarise(Count = n()) %>%
  mutate(Percentage = Count / sum(Count) * 100) %>%
  arrange(Q12.Gender, Q1.AI_knowledge)
pivot_table <- table_data %>%
  select(-Count) %>%
  pivot_wider(names_from = Q12.Gender, values_from = Percentage, values_fill = list(Percentage = 0))


# Add additional columns from the second photo
additional_data <- data.frame(
  Q1.AI_knowledge = c("very well informed", "well informed", "sufficiently informed", "poorly informed", "very poorly informed/uninformed"),
  Male_Additional = c(4.3, 23.6, 41.3, 27.0, 3.8),
  Female_Additional = c(0.8, 11.2, 31.9, 41.7, 14.4),
  Total_Additional = c(2.4, 16.8, 36.1, 35.1, 9.6)
)

# Merge the tables
final_table <- pivot_table %>%
  left_join(additional_data, by = "Q1.AI_knowledge")
...
final_table %>% 
  select(Q1.AI_knowledge, F, M,, Male_Additional, Female_Additional, Total_Additional) %>%
  rename("AI Knowledge Level" = Q1.AI_knowledge, "Female (%)" = F, "Male (%)" = M, "Male (Paper)" = Male_Additional, "Female (Paper)" = Female_Additional, "Total (Paper)" = Total_Additional) %>%
  kable(format = "markdown", col.names = c("AI Knowledge Level", "Female (%)", "Male (%)", "Female (Paper)", "Male (Paper)", "Total (Paper)")) %>%
...
pivot_table3 %>% 
  kable( format = "markdown", col.names = c("AI will rull society/destroy humanity", "Female (%)", "Male (%)")) %>% 
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
```
