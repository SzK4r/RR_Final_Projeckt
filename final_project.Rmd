...
selected_data <- data$Q1.AI_knowledge[, selected_columns]
...
colnames(data) <- c('Q3.1.AI_dehumanization', 'Q3.2.Job_replacement', 'Q3.3.Problem_solving', 'Q3.4.AI_rulling_society',
...
data_subset_long$variable <- factor(data_subset_long$variable, levels = colnames(data), labels = labels)
...
fig <- plot_ly(data, labels = ~categories, values = ~values, type = 'pie',
...
```{r}
feelings <- c("Curiosity", "Fear", "Indifference", "Trust")
percentages <- c(68.1, 13.2, 11, 7.7)
values <- percentages / 100 * sum(percentages)  # Assuming total sum of percentages as a whole
...
# Labels with percentages
labels <- paste(percentages, "% ", feelings, sep="")

# Create the pie chart with enhanced appearance
pie(values, labels = labels, col = custom_colors, border = 'white', 
    init.angle = 90, clockwise = TRUE, main = "Students' Feelings about the Development of AI")

# Optionally, add a legend if required
legend("topright", legend = feelings, fill = custom_colors,
       bty = "n", cex = 0.8)
```


```{r}
data3 <- data[c('Q1.AI_knowledge', 'Q3.2.Job_replacement', 'Q3.3.Problem_solving', 'Q3.4.AI_rulling_society', 'Q12.Gender')]
...
ggplot(knowledge_summary, aes(x = "", y = Percentage, fill = Category)) +
